...
export class HStreamScraper extends BaseScraper {
  ...
  async getStreamLink(slug: string): Promise<StreamInfoDto> {
    try {
      ...
      const apiRes = await this.client
        .post('player/api', {
      ...
      const streamDomains =
        apiRes.stream_domains ?? apiRes.asia_stream_domains ?? [];
      const domain = (streamDomains[0] ?? '').replace(/\/$/, '');
  ...
  private parseList(html: string): VideoBaseDto[] {
    ...
    $('[wire\\:key^="episode-"]').each((_, el) => {
      ...
      const title =
        $el.find('img').attr('alt')?.trim() || slug.replace(/-/g, ' ');
